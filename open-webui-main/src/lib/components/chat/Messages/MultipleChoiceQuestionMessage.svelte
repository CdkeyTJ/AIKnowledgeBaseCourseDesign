<script lang="ts">
	...
	function submit() {
		...
		const correct = [...message.answer].sort();
	...
	// 动态类名
	$: feedbackClass = feedback.includes('全部选择正确')
		? 'correct'
...
<div class="bg-muted p-4 rounded-lg shadow w-full space-y-4">
	<div
		class="question text-base font-semibold"
		style="color: var(--foreground); margin-bottom: 1rem;"
	>
		{message.question}
	</div>
	...
	{#each message.options as option, i}
		<button
			...
			class:correct={submitted && message.answer.includes(i)}
			class:wrong={submitted && !message.answer.includes(i) && selectedIndices.includes(i)}
	...
	{#if !submitted}
		<button
			class="submit-btn"
			on:click={submit}
			disabled={selectedIndices.length === 0 || readOnly}
		>
			提交答案
		</button>
	{:else}
		...
		{#if feedback.includes('不完全') || feedback.includes('完全不正确')}
			<div class="correct-answer">
				正确答案是：
				<strong>{message.answer.map((i) => message.options[i]).join('，')}</strong>
			</div>
		{/if}
...
<style>
	...
	button.choice-button {
		...
		color: black;
		padding: 7px 16px; /* 减少了上下的内边距，从 8px 变为 4px */
		margin-bottom: 8px;
		width: 100%;
		text-align: left;
		cursor: pointer;
